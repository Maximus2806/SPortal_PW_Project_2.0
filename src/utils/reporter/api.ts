...
export const logApiRequest = async (requestOptions: IRequestOptions) => {
  await allure.step(
    `Request: ${requestOptions?.method?.toUpperCase()} ${requestOptions?.url}`,
    async () => {
...
async <T extends IResponseFields>(requestOptions: IRequestOptions, response: IResponse<T>)  => {
  await allure.step(
    `Response: ${response?.status} status, ${requestOptions?.url}`,
    async () => {
      await allure.attachment(
        'Response Headers',
