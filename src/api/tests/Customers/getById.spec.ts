...
test.describe('[API] [Customers] Get customer by Id', async function () {
  ...
  test('Should get customer by existing Id', { tag: [TAGS.SMOKE] }, async function ({ customersApiService }) {
  ...
  test(
    'Should return 404 error for valid but non existing Id',
    { tag: [TAGS.REGRESSION] },
    async function ({ signInApiService, customersController }) {
  ...
  //skip untill fixed
  test.skip(
    'Should return 400 error for invalid Id',
    { tag: [TAGS.REGRESSION] },
    async function ({ signInApiService, customersController }) {
  ...
  test(
    'Should return 401 error for expired auth token',
    { tag: [TAGS.SMOKE] },
    async function ({ customersApiService, customersController }) {
      const expiredToken =
