...
test.describe('[API] [Customers] [Get all customers]', async function () {
  ...
  test(
    'Should get all customers with valid query parameters',
    { tag: [TAGS.SMOKE] },
  ...
  test(
    'Should get all customers without query parameters',
    { tag: [TAGS.SMOKE] },
    async function ({ customersApiService }) {
  ...
  test(
    'Should not get customers without providing auth token',
    { tag: [TAGS.SMOKE] },
  ...
  test('Should not get customers with expired token', { tag: [TAGS.SMOKE] }, async function ({ customersController }) {
    const expiredToken =
  ...
  test(
    'Should find customer by full or partial email',
    { tag: [TAGS.REGRESSION] },
    async function ({ customersApiService }) {
  ...
  test(
    'Should find customers by coincidense in several fields at the same time',
    { tag: [TAGS.REGRESSION] },
    async function ({ customersApiService }) {
      const user1 = {
  ...
  test(
    'Should find customers by search term in different fields with case sensitivity check',
    { tag: [TAGS.REGRESSION] },
    async function ({ customersApiService }) {
  ...
  test(
    'Should receive empty array if no customers found',
    { tag: [TAGS.REGRESSION] },
    async function ({ customersApiService }) {
  ...
  test('Should find customers by single country', { tag: [TAGS.REGRESSION] }, async function ({ customersApiService }) {
  ...
  test('Should sort customers by email', { tag: [TAGS.REGRESSION] }, async function ({ customersApiService }) {
    const customers = [
  ...
  test('Should sort customers by name', { tag: [TAGS.REGRESSION] }, async function ({ customersApiService }) {
    const customers = [
