...
import { createBodyData } from '../../../data/api/Customers/createBodyData';
...
test.describe('[API] [Customers] [Create New Customer]', async function () {
  ...
  test(
    'Should create new customer with random valid data and all fields',
    { tag: [TAGS.SMOKE] },
  ...
  test(
    'Should create new customer with random valid data and only required fields',
    { tag: [TAGS.SMOKE] },
    async function ({ customersApiService }) {
      const customerDataReqiuired = _.omit(generateNewCustomer(), ['notes']);
      const createCustomerResponse = await customersApiService.create(customerDataReqiuired);
      expect(createCustomerResponse).toMatchObject({
        ...customerDataReqiuired
      });
      await customersApiService.delete(createCustomerResponse._id);
    }
  );
  ...
  test(
    'Should create new customer with random valid data and one excessive field',
    { tag: [TAGS.REGRESSION] },
    async function ({ customersApiService }) {
      const customerData = generateNewCustomer();
  ...
  test(
    'Should not create new customer with not unique email',
    { tag: [TAGS.SMOKE] },
    async function ({ customersApiService, customersController }) {
  ...
  test('Should return 400 error for empty body', { tag: [TAGS.REGRESSION] }, async function ({ customersController }) {
  ...
  test(
    'Should return 401 error for invalid auth token',
    { tag: [TAGS.SMOKE] },
    async function ({ customersController }) {
      const customerData = generateNewCustomer();
  ...
  test(
    'Shoud verify createdOn field format in response',
    { tag: [TAGS.REGRESSION] },
    async function ({ customersApiService }) {
      const customerData = generateNewCustomer();
      const createCustomerResponse = await customersApiService.create(customerData);
  ...
  requiredFields.forEach((field) =>
    test(
      `Should not create customer without ${field} field`,
      { tag: [TAGS.REGRESSION] },
      async function ({ customersController }) {
  ...
  createBodyData.forEach(({ description, params, expectedStatus, isSuccess, errorMessage }) => {
    test(
      `${description}`,
      { tag: [TAGS.REGRESSION] },
      async function ({ signInApiService, customersController, customersApiService }) {
