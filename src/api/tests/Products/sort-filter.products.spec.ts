...
test.describe('[API] [Products] Products sort and filtering', { tag: [TAGS.REGRESSION] }, async () => {
  ...
  let p1: IProductFromResponse, p2: IProductFromResponse, p3: IProductFromResponse,
    p4: IProductFromResponse, p5: IProductFromResponse;
    ...
    for (const order of Object.values(ESortOrder)) {
      test(
        ...
        async function ({ productApiService }) {
          ...
          const isSorted = sortedResponse.every((p, i) =>
            p[key as keyof IProductFromResponse] === products[i][key as keyof IProductFromResponse]
          );
  ...
  test(`Should get product matching partial name search`, async ({ productApiService }) => {
    ...
    expect(filteredProducts.every((prod) => prod.name === p3.name),
      `Should contain product in the list with name '${p3.name}'`).toBe(true);
  });
  ...
  test(`Should get products filtred by single manufacturer`, async ({ productApiService }) => {
    ...
    expect(filteredProducts.every((prod) => prod.manufacturer === p1.manufacturer),
      `Should contain products in the list with manufacturer '${p1.manufacturer}'`).toBe(true);
  });
  ...
  test(`Should get products filtred by multiple manufacturer`, async ({ productApiService }) => {
    ...
    expect(filteredProducts.every((prod) => manufacturers.includes(prod.manufacturer)),
      'Should contain products in the list').toBe(true);
  });
